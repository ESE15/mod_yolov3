...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while (1)
    {
        ...
        draw_detections(im, dets, nboxes, thresh, names, alphabet, l.classes, NULL);
        free_detections(dets, nboxes);
        if (outfile)
        {
...
void run_detector(int argc, char **argv)
{
    ...
    char *datacfg2 = (argc > 7) ? argv[7] : 0;
    char *cfg2 = (argc > 8) ? argv[8] : 0;
    ...
    else if (0 == strcmp(argv[2], "demo"))
    {
        ...
        char **names2;
        if(argc>9){
            options2 = read_data_cfg(datacfg2);
            classes2 = option_find_int(options2, "classes", 20);
            name_list2 = option_find_str(options2, "names", "data/names.list");
            names2 = get_labels(name_list2);
        }
        if(argc>7){
            demoMod(cfg, weights, thresh, cam_index, filename, names, classes, frame_skip, prefix, avg, hier_thresh, width, height, fps, fullscreen, cfg2,weights2,names2,classes2);
        }
        else{
